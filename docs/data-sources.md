...
- For the 100k bootstrap, continue the already-checkpointed year/axis relevance pages in large bounded batches before
  falling through to cursor-paged `title_and_abstract.search` if a slice reaches the page-number ceiling. The worker
  round-robins the six local AI × MOT axes by publication year, stores each raw provider page as ignored gzip JSON, and
  applies the same local taxonomy gate before canonical import. This preserves a replayable bulk layer without
  mirroring the multi-hundred-GB OpenAlex works snapshot onto the Mac mini.
