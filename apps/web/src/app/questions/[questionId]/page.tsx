...
export default async function QuestionDetailPage({ params }: { params: Promise<{ questionId: string }> }) {
  ...
  return <>
    ...
    <section className="grid">
      ...
      <article className="card span6"><h3 className="sectionTitle">What to read next</h3><p className="muted">Recommendations combine question relevance, corpus-local citation paths, and unread novelty. Citation count is not used as a quality proxy, and read/archived papers are excluded by default.</p><div className="noteStack">{recommendations.length ? recommendations.map((paper) => <article className="questionCard" key={paper.id}><Link className="textLink" href={`/library/${paper.id}`}><strong>{paper.title}</strong></Link><small>{paper.publication_year ?? "—"} · score {paper.score.toFixed(3)} · semantic: {paper.semantic_provider}</small><div className="rankRow"><span className="pill">Query #{paper.query_rank ?? "—"}</span><span className="pill">Backward seeds {paper.backward_seed_count}</span><span className="pill">Forward seeds {paper.forward_seed_count}</span><span className="pill">Reading {paper.reading_status ?? "unqueued"}</span></div><details><summary>Why this recommendation?</summary><div className="rankRow">{Object.entries(paper.score_components).map(([name, value]) => <span className="pill" key={name}>{name}: {value.toFixed(3)}</span>)}</div><p className="muted">{paper.reasons.join(" · ")}</p></details><form action={linkEntityAction.bind(null, q.id, "papers")}><input type="hidden" name="entity_id" value={paper.id} /><button className="button buttonSecondary" type="submit">Link to this question</button></form></article>) : <span className="muted">No unlinked recommendation is available yet.</span>}</div></article>
