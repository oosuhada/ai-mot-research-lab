    ...
    <>
      ...
      <section className="evidenceDepthLedger" aria-label="Corpus evidence depth">
        ...
        <div className="evidenceDepthGrid">
          ...
          <article><span><LocalizedText en="Queue" ko="대기열" /></span><strong>{(coverage?.full_text_queued ?? landscape?.full_text_queued ?? 0).toLocaleString()}</strong><p><LocalizedText en="Full-text waiting" ko="전문 보강 전체 대기" /></p><small><LocalizedText en={`Ready ${(coverage?.full_text_claimable ?? 0).toLocaleString()} · retry delay ${(coverage?.full_text_deferred ?? 0).toLocaleString()} · processing ${(coverage?.full_text_processing ?? 0).toLocaleString()} · completed 24h ${(coverage?.full_text_completed_24h ?? 0).toLocaleString()}`} ko={`즉시 처리 ${(coverage?.full_text_claimable ?? 0).toLocaleString()} · 재시도 대기 ${(coverage?.full_text_deferred ?? 0).toLocaleString()} · 처리 중 ${(coverage?.full_text_processing ?? 0).toLocaleString()} · 24시간 완료 ${(coverage?.full_text_completed_24h ?? 0).toLocaleString()}`} /></small></article>
