    ...
    <>
      ...
      <section className="card">
        <form className="filterForm" action="/library" method="get">
          ...
          <select className="select" name="semantic_provider" defaultValue={semanticProvider}><option value="auto">Semantic: auto (recommended)</option><option value="local_hash">Semantic: local_hash baseline</option><option value="fastembed">Semantic: MiniLM neural local</option></select>
        ...
        {!query ? <div className="emptyState" style={{ marginTop: 18 }}>Search the 529-paper local corpus or import your own records and permitted PDFs.</div> : result ? (
          <div className="resultStack"><div className="resultSummary"><strong>{result.total} ranked papers</strong><span className="pill">{result.mode}</span><span className="pill">semantic: {result.semantic_provider}</span><span className="pill">requested: {result.semantic_provider_requested}</span><span className="pill">{result.semantic_provider_reason}</span><span className="pill">reranker: {result.reranker}</span><span className="pill">scope: {result.scope}</span><span className="pill">sort: {result.sort}</span></div>
