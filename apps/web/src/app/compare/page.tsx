        ...
        <section className="comparisonNotebook">
          <div className="comparisonNotebookHeader">
            ...
            <div className="comparisonSummaryActions">
              <Link className="textLink" href={`/chat?scope=comparison_set&ids=${comparison.id}`}><LocalizedText en="Ask about this comparison →" ko="이 비교에 대해 질문하기 →" /></Link>
              <a className="textLink" href={`/api/exports/comparison/${comparison.id}?format=markdown`}><LocalizedText en="Export Markdown ↗" ko="Markdown 내보내기 ↗" /></a>
              <a className="textLink" href={`/api/exports/comparison/${comparison.id}?format=csv`}><LocalizedText en="Export CSV ↗" ko="CSV 내보내기 ↗" /></a>
            </div>
          </div>
                          ...
                          <>
                            ...
                            {!readOnly ? (
                              <details className="comparisonEdit">
                                <summary><LocalizedText en="Add / revise user note" ko="사용자 노트 추가 / 수정" /></summary>
                                <form action={editComparisonCellAction.bind(null, comparison.id, cell.id)} className="formStack">
                                  ...
                                  <button className="button" type="submit"><LocalizedText en="Save cell" ko="셀 저장" /></button>
                                </form>
                              </details>
                            ) : null}
                          </>
        ...
        <article className="comparePickerPanel">
              ...
              {selectedPapers.map((paper, index) => {
                ...
                return (
                  <div className="selectedPaperRow" key={paper.id}>
                    ...
                    <Link className="textLink" href={pickerHref(nextIds, pickerQuery)}><LocalizedText en="Remove" ko="제거" /></Link>
                  </div>
                );
              })}
            </div>
          ...
          {selectedIds.length ? (
            <div className="compareSelectionActions">
              <Link className="secondaryButton" href={`/chat?scope=papers&ids=${encodeURIComponent(selectedIds.join(","))}`}><LocalizedText en="Ask selected papers with evidence →" ko="선택 논문에 근거로 질문하기 →" /></Link>
            </div>
          ) : null}
          ...
          ) : <div className="readOnlyPanel"><strong><LocalizedText en="Public Demo · Read-only" ko="공개 데모 · 읽기 전용" /></strong><span><LocalizedText en="Use an existing comparison above to inspect the evidence matrix. Creating new sets is disabled on the portfolio deployment." ko="위의 기존 비교를 사용해 근거 매트릭스를 확인하세요. 포트폴리오 배포 환경에서는 새 비교 세트 생성이 비활성화됩니다." /></span></div>}
        </article>
        ...
        <aside className="compareTopicPanel">
          ...
          {!readOnly ? (
            <form className="formStack" action={createComparisonFromTopic}>
              <label className="compactFieldLabel"><span><LocalizedText en="Research topic" ko="연구 주제" /></span><input className="input" name="query" required minLength={2} placeholder="AI capability and firm performance" /></label>
              <button className="button buttonSecondary" type="submit"><LocalizedText en="Compare top 3 candidates" ko="상위 후보 3편 비교" /></button>
            </form>
          ) : <Link className="secondaryButton" href="/library"><LocalizedText en="Select papers in Library →" ko="라이브러리에서 논문 선택 →" /></Link>}
        </aside>
      </section>
    </>
  );
