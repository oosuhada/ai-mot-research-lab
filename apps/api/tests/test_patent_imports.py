...
KOREAN_WIPS_CSV = "\n".join(
    [
        ...
        (
            "KR,10-2025-0012345,KR20260012345A,생성형 AI 기반 기술경영 분석 시스템,"
            "기술 정보를 분석한다,2025.02.03,2026.01.15,성균관대학교;테스트기업,홍길동;김연구,"
            "G06F 40/30;G06Q 10/06,G06F40/30,공개"
        ),
        "",
    ]
)

ENGLISH_WIPS_CSV = """Publication Number,Title,Filing Date,Applicant,Inventor,IPC,Legal Status
...
def test_parse_wips_csv_with_korean_headers() -> None:
    rows = parse_wips_csv(KOREAN_WIPS_CSV)
    assert len(rows) == 1
    row = rows[0]
...
def test_parse_wips_csv_with_english_headers_and_jurisdiction_inference() -> None:
    rows = parse_wips_csv(ENGLISH_WIPS_CSV)
    row = rows[0]
