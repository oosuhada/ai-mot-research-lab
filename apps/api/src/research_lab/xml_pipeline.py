...
class XmlEvidenceService:
    ...
    def ingest(
        ...
        *,
        ...
        blob_id, target = sharded_private_blob(private_root, paper_id, digest, "xml")
        if self.settings.private_data_require_external:
            require_private_blob_shard(target.parent)
        else:
