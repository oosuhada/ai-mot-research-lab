...
run_worker pmc-bulk enrich-full-text-pmc-bulk \
  --max-items "${PRO_FULL_TEXT_PMC_MAX_ITEMS:-260}" \
  --download-workers "${PRO_FULL_TEXT_PMC_DOWNLOAD_WORKERS:-10}" \
...
run_worker arxiv enrich-full-text \
  --source-lane arxiv \
  --max-items "${PRO_FULL_TEXT_ARXIV_MAX_ITEMS:-45}" \
  --max-pdf-bytes 30000000 \
  --lease-minutes 20 \
  --worker-id "pro:arxiv:${HOST:-pro}:$$"

for worker_index in {1..4}; do
